...
results_df = pd.read_csv("data/learn-compare.csv")
...
# Accuracy plot
acc_plot = results_df.pivot(index="Model", columns="Method", values="Accuracy").plot(
    kind="bar", ax=axes[0]
)
...
# Precision plot
prec_plot = results_df.pivot(index="Model", columns="Method", values="Precision").plot(
    kind="bar", ax=axes[1]
)
