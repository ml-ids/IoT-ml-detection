...
def calculate_metrics(y_test, y_pred, model_name, balance_method, results):
    accuracy = accuracy_score(y_test, y_pred)
    precision = precision_score(y_test, y_pred, average="weighted")
    f1 = f1_score(y_test, y_pred, average="weighted")

    results.append((model_name, balance_method, accuracy, precision, f1))
    print(f"{model_name} ({balance_method} Balanced): " f"Accuracy = {accuracy:.8f}, Precision = {precision:.8f}, F1-Score = {f1:.8f}")  # print with 8 digit precision
...
models_scaling_needed = [("Logistic Regression", LogisticRegression(max_iter=LR_MAX_ITER))]  # models that need scaling, StandardScaler in this case
